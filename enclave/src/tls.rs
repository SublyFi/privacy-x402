...
impl TlsRuntime {
    pub fn from_env() -> Result<Option<Self>, String> {
        ...
        let cert_path =
            cert_path.ok_or_else(|| "A402_ENCLAVE_TLS_CERT_PATH must be set when TLS is enabled".to_string())?;
        let key_path =
            key_path.ok_or_else(|| "A402_ENCLAVE_TLS_KEY_PATH must be set when TLS is enabled".to_string())?;
...
pub async fn serve(listener: TcpListener, app: Router, runtime: TlsRuntime) -> io::Result<()> {
    info!(
        mtls_enabled = runtime.mtls_enabled,
        "Enclave TLS listener enabled"
    );

    loop {
        let (tcp_stream, remote_addr) = listener.accept().await?;
        let acceptor = runtime.acceptor.clone();
        let app = app.clone();
        ...
        tokio::spawn(async move {
            let tls_stream = match acceptor.accept(tcp_stream).await {
                Ok(stream) => stream,
                Err(error) => {
                    warn!(%remote_addr, "failed to complete TLS handshake: {error}");
                    return;
                }
            };
            ...
            if let Err(error) = HyperServerBuilder::new(TokioExecutor::new())
                ...
                .await
            {
                warn!(%remote_addr, "failed to serve TLS connection: {error}");
            }
        });
    }
}
...
fn load_certs(path: &str) -> Result<Vec<CertificateDer<'static>>, String> {
    let file = File::open(path).map_err(|error| format!("failed to open TLS cert {path}: {error}"))?;
...
fn load_private_key(path: &str) -> Result<PrivateKeyDer<'static>, String> {
    let file = File::open(path).map_err(|error| format!("failed to open TLS key {path}: {error}"))?;
    let mut reader = BufReader::new(file);
...
fn load_root_store(path: &str) -> Result<RootCertStore, String> {
    ...
    if added == 0 {
        return Err(format!("TLS client CA bundle {path} did not contain a parsable certificate"));
    }
    Ok(store)
}
