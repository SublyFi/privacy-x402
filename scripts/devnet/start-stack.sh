...
if [[ ! -x target/debug/a402-watchtower || ! -x target/debug/a402-enclave ]]; then
  NO_DNA=1 cargo build -p a402-watchtower -p a402-enclave
fi
...
if [[ -f data/watchtower-devnet.pid ]] && kill -0 "$(cat data/watchtower-devnet.pid)" 2>/dev/null; then
  ...
  NO_DNA=1 nohup target/debug/a402-watchtower > data/logs/watchtower-devnet.log 2>&1 < /dev/null &
...
if [[ -f data/enclave-devnet.pid ]] && kill -0 "$(cat data/enclave-devnet.pid)" 2>/dev/null; then
  ...
  NO_DNA=1 nohup target/debug/a402-enclave > data/logs/enclave-devnet.log 2>&1 < /dev/null &
