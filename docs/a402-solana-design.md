...
| Provider Integration | A402専用プロトコル | x402 HTTP envelope上に `a402-svm-v1` scheme を載せる | API統合をHTTP 402に寄せ、導入コストを下げる |
...
- x402互換のfetch（内部で `a402-svm-v1` payload を生成）
- 監査ツール
...
- Providerは `PAYMENT-REQUIRED` で `accepts[].scheme = "a402-svm-v1"` を返す
...
| `PAYMENT-REQUIRED` | 402レスポンス | 402レスポンス | `accepts[].scheme` が `a402-svm-v1` |
...
Phase 1-2では、x402のHTTP envelopeを維持しつつ、client が `a402-svm-v1` payload をローカル生成し、payment verification / reservation / settlementはNitro Enclave facilitatorが担当する。Provider API本体は変更しないが、**既存の汎用facilitatorは利用しない**。アトミシティは依然としてProviderの信頼に依存する。

```
1. Client: `PAYMENT-REQUIRED` から `paymentDetailsHash` を計算し、request hash を含む `a402-svm-v1` payload をローカル生成
...
fn submit_batch(
    ...
    audits: Vec<AuditRecordData>,
) {
    if audits.is_empty() {
        for chunk in settlements.chunks(MAX_SETTLEMENTS_PER_TX_PHASE1) {
            submit_settle_vault_tx(batch_id, chunk);
        }
        return;
    }

    assert_eq!(settlements.len(), audits.len()); // 1 settlement entry <-> 1 audit record

    for (settlement_chunk, audit_chunk) in settlements
        .chunks(MAX_ATOMIC_SETTLEMENTS_PER_TX_WITH_AUDIT)
        .zip(audits.chunks(MAX_ATOMIC_SETTLEMENTS_PER_TX_WITH_AUDIT))
    {
...
既存x402の `fetch → 402処理 → PAYMENT-SIGNATURE再送` という利用体験を維持しつつ、内部ではNitro Enclave向けの `a402-svm-v1` payload を生成する。
