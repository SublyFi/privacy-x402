...
- `authMode` は Phase 1 では `bearer` または `mtls`
...
3. `paymentDetailsHash` が一致する
4. `requestHash` が `requestContext` から再計算した値と一致する
5. `clientSig` が有効
6. `expiresAt` が未来
7. `providerId`, `payTo`, `assetMint`, `network`, `vault` が registration / vault config と一致する
8. client の `free_balance >= amount`
9. `paymentId` が未使用、または同一 request への idempotent replay である
10. vault status が `Active` である
